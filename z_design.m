...
z_d = exp(s_d * T); % Mapping to discrete z-plane
...
angle_sum = angle_sum - rad2deg(angle(z_d - z_c));
p_c = real(z_d) - imag(z_d) / tand(-angle_sum);

% Calculate the compensated gain
z = [z; z_c];
p = [p; p_c];

len_z = prod(abs(z_d - z)); % Cumulative zero modules
len_p = prod(abs(z_d - p)); % Cumulative pole modules

kc = 1 / (k * len_z / len_p);

Gcz = kc * tf([1 -z_c], [1 -p_c], T); % Compensator designed
display(Gcz);
